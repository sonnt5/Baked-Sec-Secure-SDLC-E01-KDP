...
| **Blocking Conditions** | (1) Issue #1: Account lockout must be implemented and tested (test: `test_account_lockout_after_5_failures` must pass) before design is approved. (2) Issue #2: Code review must confirm `Depends(verify_submission_owner)` is present on ALL submission endpoints — provide code diff as evidence. (3) Issue #3: Backend RBAC for admin destructive operations must be designed (2-person approval workflow per SR-D03 — not in original SRS, added during SDR) — provide design doc for approval workflow. |
| **Non-Blocking Requirements** | HIGH: Issue #4 (sandbox verification — add CIS benchmark to security test plan). HIGH: Issue #5 (verify CDR-003 uses RS256 per SDD 3.1, not ES256). HIGH: Issue #8 (add DELETE-denied test for admin_audit_logs table immutability). MEDIUM: Issues #6, #7. |
| **Strengths** | (1) Envelope Encryption design is architecturally sound — DEK/CMK separation provides strong defense in depth; TDE at volume level (per SDD 7.2) adds physical protection. (2) Interface Catalogue demonstrates security thinking was applied from the beginning of design, not bolted on. (3) TLS configuration (nginx config per SDRD 7.1) is thorough — TLS 1.3, HSTS, OCSP stapling, session tickets disabled. (4) 4 of 6 assumptions are well-documented with verification methods and fallbacks — good traceability practice. (5) Strong cryptographic design: RS256 JWT, Argon2id password hashing, network isolation for judge. |
...
| **Executive Summary** | CODING WAR's design was reviewed covering authentication, submission pipeline, judging, and admin interfaces. Three critical design issues were identified: missing account lockout implementation, unconfirmed IDOR protection, and insufficient admin access controls (2-person approval needed for destructive operations). Design approval is deferred until these three issues are resolved — estimated 1–2 sprints. The cryptographic architecture (RS256 JWT per SDD 3.1, Argon2id per SDD 3.1, TDE per SDD 7.2) and TLS configuration (HSTS, OCSP stapling per SDRD 7.1) are well-designed and do not require revision. |
