...
**Data Stores (≥4):** PostgreSQL DB, Redis Cache, RabbitMQ Queue, MinIO Object Storage

**Trust Boundaries (≥4):**
- B1: Internet → Nginx/Web Tier
- B2: Web Tier → Application Services
- B3: Application Services → Data Tier (DB, Cache, Queue)
- B4: Application Services → JudgeWorker (separate execution environment)
...
|----|------|-------------|---------------------|--------------|
| **7** | Problem Setter | Authenticated user with Problem Setter role — can create problems but not manage users | EP-1.6 (create problem) | Asset 3 (Test Cases), Asset 2 (Submissions — read only during contest) |
| **8** | Object Storage Service Account | MinIO/S3 account used by the application to read/write test case files | \[Internal B3 boundary\] | Asset 3 (Test Cases — full access) |
| **9** | Secret Manager | Vault/AWS Secrets Manager service that holds credentials | \[Internal — accessed at startup\] | Asset 4 (JWT Secret), Asset 6 (DB Connection Strings) |
...
| **1.9** | Problem Management API | POST/PUT/DELETE /api/v1/problems/* | (3) Admin, (7) Problem Setter | HIGH |
| **3** | Object Storage (MinIO) | Internal API — JudgeService reads test case files | (4) Judge Engine, (8) Storage SA | HIGH |
