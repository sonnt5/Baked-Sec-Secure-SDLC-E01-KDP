...
| Anonymous user scrapes all problem statements to build a competing platform | I4 | Automated bot | GET /api/v1/problems with high frequency | Rate limit: 30/min/IP | No per-session tracking — distributed scraping across many IPs bypasses per-IP limit |
...
| **Audit Requirements** | Every reset request logged: `actor_ip_hash`, `email_hash` (not raw), `timestamp`, `token_used: bool`; alert if >3 requests/hour from same email hash |
